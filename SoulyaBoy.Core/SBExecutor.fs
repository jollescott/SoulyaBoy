...
module internal SBExecutor =
    ...
    let private INTERRUPT_ADDRESSES = Map([ (0, 0x0040us) ])

    let private IncrementPC pcd =
        sb {
            let! mb = SB.Get
            do! SB.Put { mb with CPU.PC = mb.CPU.PC + 1us + pcd }
        }

    let private UpdateIMEFlag =
        sb {
            let! mb = SB.Get

            let handledMB =
                match mb.CPU.IME with
                | Disable -> { mb with CPU.IME = Disabling }
                | Disabling -> { mb with CPU.IME = Disabled }
                | Enable -> { mb with CPU.IME = Enabling }
                | Enabling -> { mb with CPU.IME = Enabled }
                | _ -> mb
    ...
    let private RetrieveOpcodeInstruction opcode (instructions: SBInstructionTable) =
        sb {
